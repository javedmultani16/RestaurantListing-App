...
public extension UITableView {
    ...
    func reloadData(withPlaceholderString placeholderString: String, lookupsection section: Int) {
        backgroundView = nil
        if numberOfSections == 0 && (numberOfRows(inSection: section) == 0 || numberOfRows(inSection: section) == NSNotFound) {
            if (placeholderString.count ) > 0 {
                let lblEmpty: UILabel? = getLabel()
                lblEmpty?.text = placeholderString
    ...
    func reloadData(withPlaceholderString placeholderString: String, with placeholderColor: UIColor) {
        backgroundView = nil
        if numberOfSections == 0 || (numberOfRows(inSection: 0) == 0 || numberOfRows(inSection: 0) == NSNotFound) {
            if (placeholderString.count ) > 0 {
                let lblEmpty: UILabel? = getLabel()
    ...
    func getLabel() -> UILabel {
       ...
       // lblEmpty.font = UIFont.systemFont(ofSize: 14)
